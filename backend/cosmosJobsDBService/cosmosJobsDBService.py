...
class CosmosJobsDBService():
    ...
    async def get_job_info(self, job_id: str):
        try:
            ...
            job_info = [item async for item in self.container_client.query_items(query, enable_cross_partition_query=True)]
            if not job_info:
                return None
            return job_info[0]
        except exceptions.CosmosHttpResponseError as e:
    ...
    async def get_job_key_skills(self, job_id: str):
        try:
            query = f"SELECT c.key_skills FROM c WHERE c.id = '{job_id}'"
            job_info = [item async for item in self.container_client.query_items(query, enable_cross_partition_query=True)]
            if not job_info:
                return None
            return job_info[0].get("key_skills", [])
        except exceptions.CosmosHttpResponseError as e:
            raise ValueError(f"Error querying CosmosDB: {str(e)}")
