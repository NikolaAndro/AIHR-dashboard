...
class CosmosCandidatesDBService():
    ...
    async def get_candidate_info(self, applied_position_id: str):
        try:
            query = f"SELECT * FROM c WHERE c.applied_position_id = '{applied_position_id}'"
            candidate_info = [item async for item in self.container_client.query_items(query)]
